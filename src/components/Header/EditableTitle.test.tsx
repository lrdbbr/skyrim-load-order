...
describe('EditableTitle', () => {
  it('shows the current title as a heading, with an edit button', () => {
    ...
    expect(
      screen.getByRole('button', { name: /modifier le titre/i }),
    ).toBeInTheDocument()
  })

  it('switches to an editable input when the pencil button is clicked', () => {
    render(<EditableTitle />)
    fireEvent.click(screen.getByRole('button', { name: /modifier le titre/i }))

    const input = screen.getByRole('textbox', { name: /titre du load order/i })
